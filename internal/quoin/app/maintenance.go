...
func NewMaintenanceHandler(service *auth.Service, database *sql.DB, publicOrigin string) (http.Handler, error) {
	return newMaintenanceHandler(NewMaintenanceAPIServer(service, database, ""), publicOrigin, "Restore")
}

// newMaintenanceHandler exposes exactly the active maintenance reason's allowlist. Its
...
// registerMaintenanceConnectionRoutes is shared by Restore and RootKeyRebind.
// It deliberately excludes probe/enable because both can create external work.
func (application *apiServer) registerMaintenanceConnectionRoutes(api huma.API) {
...
func (application *apiServer) maintenanceChangePassword(ctx context.Context, input *passwordInput) (*noContentOutput, error) {
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("暂时无法读取维护状态，请重试。", err)
	}
