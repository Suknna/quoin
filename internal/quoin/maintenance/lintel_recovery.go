...
//
// The finalizer runs through the shared execution authority: one registered
...
// rollback when anything — including the audit write — fails. A receipt
// replay is a pure read: no state transition, no new audit row.
...
func FinalizeLintelRecovery(ctx context.Context, request LintelRecoveryFinalizeRequest) (LintelRecoveryFinalizeResult, error) {
	...
	defer lock.Close()
	databasePath := filepath.Join(request.DataDirectory, "quoin.db")
	// Same minimal controlled boundary as the rebind: the frozen verification
	// pass runs on an actual read-only SQLite open; only the mutation opens
	...
	defer db.Close()
	return finalizeLintelRecoveryOn(runCtx, db, request)
}
...
func finalizeLintelRecoveryOn(ctx context.Context, db *sql.DB, request LintelRecoveryFinalizeRequest) (LintelRecoveryFinalizeResult, error) {
	...
	var result LintelRecoveryFinalizeResult
	_, err := execution.Execute(ctx, runner, ops.finalize, func(tx *execution.Tx) (int64, error) {
		var active int
		var reason string
		var revision int64
		...
		if active == 0 {
			// Maintenance already exited: the receipt — if one matches — is
			// the replay authority. A pure read, no state transition, and as
			// before the runner no new audit row.
			replay, err := alreadyFinalizedResult(ctx, tx, request)
			if err != nil {
				return 0, err
			}
			result = replay
		...
		if reason != "LintelRecovery" {
			return 0, fmt.Errorf("%w: maintenance %s is active", ErrLintelRecoveryNotReady, reason)
		}
		// The frozen fence must match the finalize evidence exactly.
		rows, err := tx.QueryContext(ctx, `SELECT object_key FROM maintenance_items WHERE maintenance_revision=? AND kind='LintelRecoveryFence'`, revision)
		if err != nil {
			return 0, err
		}
		...
		if len(frozen) != len(expected) || !sameKeys(frozen, expected) {
			return 0, fmt.Errorf("%w: frozen fence digests do not match the finalize evidence", ErrLintelRecoveryNotReady)
		}
		...
		if err := tx.QueryRowContext(ctx, `SELECT state,current_credential_id,pending_credential_id,retiring_credential_id FROM runtime_slots WHERE slot='lintel'`).Scan(&state, &currentID, &pendingID, &retiringID); err != nil {
			return 0, err
		}
		if state != "registered" || pendingID.Valid || retiringID == 0 {
			return 0, fmt.Errorf("%w: lintel slot is not in the post-registration recovery shape", ErrLintelRecoveryNotReady)
		}
		var oldGen int64
		var oldRetired sql.NullString
		if err := tx.QueryRowContext(ctx, `SELECT generation,retired_at FROM runtime_credentials WHERE id=?`, retiringID).Scan(&oldGen, &oldRetired); err != nil {
			return 0, err
		}
		var replGen int64
		var replConfirmed, replFirstAuth, replRetired sql.NullString
		if err := tx.QueryRowContext(ctx, `SELECT generation,confirmed_at,first_authenticated_at,retired_at FROM runtime_credentials WHERE id=?`, currentID).Scan(&replGen, &replConfirmed, &replFirstAuth, &replRetired); err != nil {
			return 0, err
		}
		if oldRetired.Valid || !replConfirmed.Valid || !replFirstAuth.Valid || replRetired.Valid {
			return 0, fmt.Errorf("%w: replacement credential lacks its first-authenticated Hello fact", ErrLintelRecoveryNotReady)
		}
		...
		if err := tx.QueryRowContext(ctx, idempotenceQuery, idempotenceArgs...).Scan(&existingDigests); err != nil && !errors.Is(err, sql.ErrNoRows) {
			return 0, err
		} else if existingDigests.Valid && existingDigests.String != request.DispositionDigest {
			return 0, ErrLintelRecoveryReceiptConflict
		}

		now := time.Now().UTC().Format(time.RFC3339Nano)
		if request.Disposition == "retired" {
			closed, demoted, closureErr := retireLintelStorageDomain(ctx, tx, now)
			if closureErr != nil {
				return 0, closureErr
			}
		...
		if _, err := tx.ExecContext(ctx, `UPDATE runtime_slots SET retiring_credential_id=NULL,row_version=row_version+1 WHERE slot='lintel' AND retiring_credential_id=? AND current_credential_id=? AND pending_credential_id IS NULL`, retiringID, currentID); err != nil {
			return 0, err
		}
		if _, err := tx.ExecContext(ctx, `INSERT INTO lintel_recovery_receipts(maintenance_revision,old_slot_id,old_runtime_credential_id,old_token_generation,replacement_runtime_credential_id,replacement_token_generation,storage_disposition,disposition_digest,fence_report_digest,recovery_report_digest,post_verify_digest,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
		...
		if _, err := tx.ExecContext(ctx, `UPDATE maintenance_items SET safe_state='Safe',detail_code='lintel_recovery_finalized',updated_at=? WHERE maintenance_revision=?`, now, revision); err != nil {
			return 0, err
		}
		if _, err := tx.ExecContext(ctx, `UPDATE maintenance_state SET active=0,reason=NULL,entered_at=NULL,entered_by_type=NULL,entered_by_id=NULL,exited_at=?,exited_by_type='deployment_helper',exited_by_id=0,row_version=row_version+1 WHERE id=1 AND active=1 AND reason='LintelRecovery' AND row_version=?`, now, revision); err != nil {
			return 0, err
		}
		// The runner writes the automatic audit row in this same transaction;
		// an audit failure rolls the whole finalization back.
		result.MaintenanceRevision = revision
		result.OldGeneration = oldGen
		result.ReplacementGeneration = replGen
		return revision, nil
...
func retireLintelStorageDomain(ctx context.Context, tx *execution.Tx, now string) (int64, int64, error) {
	closed, err := tx.ExecContext(ctx, `
	...
	terminalClosed, err := tx.ExecContext(ctx, `
	...
	demoted, err := tx.ExecContext(ctx, `UPDATE browser_identities SET state='AuthenticationRequired',row_version=row_version+1 WHERE state='Ready'`)
	if err != nil {
		return 0, 0, err
	}
...
func alreadyFinalizedResult(ctx context.Context, tx *execution.Tx, request LintelRecoveryFinalizeRequest) (LintelRecoveryFinalizeResult, error) {
	...
	err := tx.QueryRowContext(ctx, query, arguments...).Scan(&revision, &oldGen, &replGen, &disposition, &dispositionDigest, &fenceDigest, &recoveryDigest, &postVerifyDigest)
