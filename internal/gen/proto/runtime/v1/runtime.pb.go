...
// 单次消费；不携带长期 Bearer（尚未拥有）。contract_fingerprint 必须是完整 Proto
// 权威集合的 canonical SHA-256（64 位小写 hex），缺失、无效或不一致均拒绝。
type RegisterRuntimeRequest struct {
	...
	ContractFingerprint string                 `protobuf:"bytes,5,opt,name=contract_fingerprint,json=contractFingerprint,proto3" json:"contract_fingerprint,omitempty"`
...
func (x *RegisterRuntimeRequest) GetContractFingerprint() string {
	if x != nil {
...
// 握手首帧：Bearer 已由 metadata 认证；本帧声明 slot/连接身份、完整 Proto
...
const file_runtime_proto_rawDesc = "" +
	"\n" +
	"\rruntime.proto\x12\x10quoin.runtime.v1\x1a\x1fgoogle/protobuf/timestamp.proto\"\xdd\x01\n" +
	...
	"\x14contract_fingerprint\x18\x05 \x01(\tR\x13contractFingerprint\"\x94\x01\n" +
	...
	"\x16ATTEMPT_OUTCOME_FAILED\x10\x02*\xff\x01\n" +
	...
	"\x1dUPLOAD_REJECT_REASON_INTERNAL\x10\b\"\x04\b\a\x10\a*\xcb\x01\n" +
	"\fGoAwayReason\x12\x1e\n" +
	"\x1aGO_AWAY_REASON_UNSPECIFIED\x10\x00\x12 \n" +
	"\x1cGO_AWAY_REASON_SHUTTING_DOWN\x10\x01\x12$\n" +
	" GO_AWAY_REASON_CONTRACT_MISMATCH\x10\x02\x12\x1a\n" +
