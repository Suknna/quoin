...
type Channel struct {
	...
	journeyOperations map[int64]int64 // attempt ID → started Browser Operation ID
	journeyCancelDone map[int64]chan struct{}
	// journeyCancelled is a per-boot cancellation tombstone. It fences a
	// DispatchAttempt which was already in flight when Quoin committed its
	// durable Cancelling state.
