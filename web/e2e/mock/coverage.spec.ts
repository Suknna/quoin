...
function protectOffline(page: import("@playwright/test").Page): Guard {
	...
	page.on("request", (request) => {
		...
		if (url.origin !== "http://127.0.0.1:4175")
			guard.forbidden.push(request.url());
	});
...
async function selectScenario(
	...
	if ((await page.getByLabel("场景").count()) === 0)
...
async function expectHealthy(
	...
	expect(
		guard.webSockets.filter((url) => new URL(url).host !== "127.0.0.1:4175"),
	).toEqual([]);
}

test("route matrix renders actual data without unhandled APIs or external transports", async ({
	page,
}) => {
...
test("login and first-password scenarios submit errors and successful auth", async ({
	page,
}) => {
	const guard = protectOffline(page);
	await page.goto("/");
...
test("mandatory mutation persists, refetches, and scenario reset restores fixtures", async ({
	page,
}) => {
	const guard = protectOffline(page);
	await page.goto("/investigations/new");
	await page
		.getByPlaceholder("描述需要调查的问题…")
		.fill("E2E mandatory local mutation");
	await page.getByRole("button", { name: "创建并发送" }).click();
	await expect(
		page.getByRole("heading", { name: "E2E mandatory local mutation" }),
	).toBeVisible();
...
test("operator controls are absent and conflict reports a failed write", async ({
	page,
}) => {
	const guard = protectOffline(page);
	await page.goto("/");
	...
	await expect(
		page.getByRole("heading", { name: "编辑知识候选" }),
	).toBeVisible();
...
test("MSW serves local EventSource and blocks business WebSocket escape", async ({
	page,
}) => {
	const guard = protectOffline(page);
	await page.goto("/alerts/list");
		...
		() =>
			new Promise<string>((resolve, reject) => {
				...
				const timeout = window.setTimeout(() => {
					source.close();
					reject(new Error("mock EventSource timed out"));
				}, 5_000);
				...
				source.onerror = () => {
					window.clearTimeout(timeout);
					source.close();
					reject(new Error("mock EventSource failed"));
				};
			}),
	);
	...
	const webSocketBlocked = await page.evaluate(() => {
		try {
			new WebSocket("wss://business.example.invalid/stream");
			return false;
		} catch (error) {
