...
if (!adminUsername || !adminPassword || !adminFinalPassword) {
	throw new Error("#96 real E2E requires generated Admin credentials from scripts/e2e-real/up.sh.");
}

async function loginAndSetPassword(page: import("@playwright/test").Page, username: string, temporary: string, final: string) {
	await page.goto("/");
	await page.getByLabel("用户名").fill(username);
	await page.getByLabel("密码").fill(temporary);
	await page.getByRole("button", { name: "登录" }).click();
	const changePassword = page.getByRole("heading", { name: "先设置你自己的密码" });
	await page.waitForTimeout(300);
	if (await changePassword.isVisible()) {
		await page.getByLabel("当前临时密码").fill(temporary);
		await page.getByRole("textbox", { name: "新密码", exact: true }).fill(final);
 ...
 */
test("Admin creates Alertmanager, Stele persists its alert, and Operator is restricted", async ({ browser, baseURL, request }) => {
	const adminContext = await browser.newContext({ ignoreHTTPSErrors: true });
	const adminPage = await adminContext.newPage();
	await loginAndSetPassword(adminPage, adminUsername!, adminPassword!, adminFinalPassword);

	const sourceKey = `e2e-alertmanager-${Date.now()}`;
	await adminPage.goto("/integrations/alertmanager");
	await expect(adminPage.getByRole("heading", { name: "配置 Alertmanager" })).toBeVisible();
	await adminPage.locator("#alertmanager-key").fill(sourceKey);
	await adminPage.getByRole("button", { name: "创建并显示一次凭据" }).click();
	await expect(adminPage.getByRole("dialog", { name: "一次性接收凭据" })).toBeVisible();
	const receiverURL = await adminPage.getByRole("dialog").locator("input[readonly]").first().inputValue();
	const bearer = await adminPage.getByRole("dialog").locator("input[readonly]").nth(1).inputValue();
	...
	await expect.poll(async () => (await request.post(receiverURL, {
		headers: { Authorization: `Bearer ${bearer}`, "Content-Type": "application/json" },
		data: { status: "firing", alerts: [] }, ignoreHTTPSErrors: true,
	})).status(), { timeout: 15_000 }).toBe(204);
	...
	const payload = {
		...
		alerts: [{
			status: "firing",
			labels: { alertname: "QuoinE2EAlert", service: "checkout", severity: "critical" },
	...
	await expect.poll(async () => {
		const response = await adminContext.request.get(`${baseURL}/api/v1/alerts`);
		const body = await response.json() as { items?: Array<{ labels?: Record<string, string> }> };
		return body.items?.some((item) => item.labels?.alertname === "QuoinE2EAlert") ?? false;
	...
	const operatorLogin = await operatorContext.request.post(`${baseURL}/api/v1/auth/login`, { headers: { Origin: baseURL }, data: { username: operatorUsername, password: operatorPassword } });
	expect(operatorLogin.status()).toBe(200);
	const operatorPasswordChange = await operatorContext.request.put(`${baseURL}/api/v1/auth/password`, { headers: { Origin: baseURL }, data: { currentPassword: operatorPassword, newPassword: operatorFinalPassword } });
	expect(operatorPasswordChange.status()).toBe(204);
	const operatorPage = await operatorContext.newPage();
	await operatorPage.goto("/alerts/list");
	await expect(operatorPage.getByText("QuoinE2EAlert", { exact: true }).last()).toBeVisible();

	for (const path of ["/api/v1/alert-sources", "/api/v1/business-systems", "/api/v1/label-contracts", "/api/v1/runtime"]) {
		const denied = await operatorContext.request.get(`${baseURL}${path}`);
		expect(denied.status(), path).toBe(403);
	}
	const context = await operatorContext.request.get(`${baseURL}/api/v1/business-context`);
