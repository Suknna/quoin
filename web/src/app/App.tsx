...
function LoginForm({
	...
	async function submit(event: FormEvent) {
		...
		setSaving(true);
		try {
			onAuthenticated(await workbenchApi.login({ username, password }));
		} catch (reason) {
			const apiError = reason as WorkbenchApiError;
			setError(
				apiError.status === 403 && apiError.code === "password_change_required"
					? "此账户必须先修改密码。"
					: messageOf(reason, "登录暂时不可用，请重试。"),
			);
		} finally {
			setSaving(false);
			setPassword("");
...
function PasswordChange({
	...
	async function submit(event: FormEvent) {
		...
		try {
			...
			setError(messageOf(reason, "没有修改成功，请重试。"));
		} finally {
			setSaving(false);
			setCurrentPassword("");
			setNewPassword("");
	...
	async function logout() {
		...
		try {
			...
			if (reason instanceof WorkbenchApiError && reason.status === 401)
				onLogout();
			else setError(messageOf(reason, "退出登录失败，请重试。"));
		}
	}
	return (
		<form className="flex flex-col gap-6" onSubmit={submit}>
			<FieldGroup>
				<div className="flex flex-col items-center gap-1 text-center">
					<h1 className="text-2xl font-bold">先设置你自己的密码</h1>
					<p className="text-sm text-balance text-muted-foreground">
						{user.displayName}，临时密码只能用于首次登录。
					</p>
				</div>
				<FieldDescription>
					使用 15–128 个字符。可以使用空格和中文。
				</FieldDescription>
				{error && <ErrorMessage>{error}</ErrorMessage>}
				<Field>
