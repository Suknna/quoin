...
describe("WorkspaceShell navigation", () => {
	...
	it("renders the persistent labelled module buttons without a switch dropdown", () => {
		renderShell("/alerts/list");
		expect(
			screen.getByRole("button", { name: "运维中心" }),
		).toBeInTheDocument();
		expect(screen.getByRole("button", { name: "AI SRE" })).toBeInTheDocument();
		expect(screen.getByRole("button", { name: "设置" })).toBeInTheDocument();
		expect(
			screen.queryByRole("button", { name: "切换模块" }),
	...
	it("shows each administrator operations entry once with only the route-matching item active", () => {
		...
		expect(
			screen.queryByRole("button", { name: "接入管理" }),
		).not.toBeInTheDocument();
		expect(screen.getByRole("button", { name: "巡检" })).toHaveAttribute(
			"aria-current",
			"page",
		);
		expect(
			screen.getByRole("button", { name: "告警列表" }),
	...
	it("keeps AI SRE knowledge navigation and active semantics", () => {
		...
		expect(screen.getByRole("button", { name: "AI SRE" })).toHaveAttribute(
			"data-active",
			"true",
		);
	});

	it("does not select a work module for the settings surface", () => {
		renderShell("/settings/profile");
		expect(screen.getByRole("button", { name: "运维中心" })).toHaveAttribute(
			"data-active",
			"false",
		);
		expect(screen.getByRole("button", { name: "AI SRE" })).toHaveAttribute(
