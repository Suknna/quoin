...
const operationItems: readonly OperationItem[] = [
	{
		title: "告警列表",
		route: "/alerts/list",
		matches: (route) => route.startsWith("/alerts"),
		icon: Bell,
	},
	{
		title: "故障复盘",
		route: "/postmortems",
		matches: (route) => route.startsWith("/postmortems"),
		icon: FileText,
	},
	{
		title: "巡检",
		route: "/inspections",
		matches: (route) => route.startsWith("/inspections"),
		icon: ClipboardCheck,
		adminOnly: true,
	},
	{
		title: "业务视图",
		route: "/business-views",
		matches: (route) => route.startsWith("/business-views"),
				...
				{operationItems
					...
					.map((item) => (
						<Button
							...
							variant={item.matches(route) ? "secondary" : "ghost"}
		...
		return (
			...
			>
				<Button
					...
					aria-current={
						route.startsWith("/investigations") ? "page" : undefined
					}
				...
				<Button
					className="w-full justify-start"
					variant={route.startsWith("/knowledge") ? "secondary" : "ghost"}
					aria-current={route.startsWith("/knowledge") ? "page" : undefined}
...
export function WorkspaceShell({
	...
	return (
		<SidebarProvider
			style={
				{
					"--sidebar-width": "calc(12.5rem + var(--sidebar-width-icon))",
				...
				<header className="sticky top-0 z-10 flex min-w-0 shrink-0 items-center gap-2 border-b bg-background p-4 md:hidden">
					...
					<Sheet>
						...
						<SheetContent side="left">
							<SheetHeader>
								<SheetTitle>{moduleHeader}</SheetTitle>
							</SheetHeader>
							{moduleList}
						</SheetContent>
					</Sheet>
				...
				>
					{!canAccessOperationsRoute(route, user) ? (
						<Alert variant="destructive">
							<AlertDescription>此页面仅向管理员开放。</AlertDescription>
						</Alert>
					) : (
						view.content
					)}
