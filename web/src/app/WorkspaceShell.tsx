...
const navigation: { title: string; route: string; icon: typeof Bell; adminOnly?: boolean }[] = [
	{ title: "告警", route: "/alerts/list", icon: Bell },
...
function isCurrent(route: string, destination: string) {
	// Postmortems belongs to the alert center even though it deliberately has a short top-level URL.
	if (destination === "/alerts/list" && route.startsWith("/postmortems")) return true;
...
export function WorkspaceShell({
	...
	const alertCenter = route.startsWith("/alerts") || route.startsWith("/postmortems");
	return <SidebarProvider style={{ "--sidebar-width": alertCenter ? "calc(14rem + var(--sidebar-width-icon))" : "350px" } as CSSProperties}>
		<Sidebar collapsible="icon" className="overflow-hidden *:data-[sidebar=sidebar]:flex-row">
			<Sidebar collapsible="none" className="w-[calc(var(--sidebar-width-icon)+1px)]! border-r">
				<SidebarHeader><SidebarMenu><SidebarMenuItem><SidebarMenuButton size="lg" onClick={() => navigate("/alerts/list")} className="md:h-8 md:p-0"><div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground"><GalleryVerticalEnd className="size-4" /></div><div className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-medium">Quoin</span><span className="truncate text-xs">工作台</span></div></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarHeader>
