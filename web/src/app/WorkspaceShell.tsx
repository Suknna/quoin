	...
	if (isAiSreRoute(route)) {
		return <nav className="flex flex-col gap-1 border-b p-3" aria-label="AI SRE 模块">
			<Button className="w-full justify-start" variant={route.startsWith("/investigations") ? "secondary" : "ghost"} aria-current={route.startsWith("/investigations") ? "page" : undefined} onClick={() => navigate("/investigations")}><SearchCheck data-icon="inline-start" />对话</Button>
				<Button className="w-full justify-start" variant={route.startsWith("/knowledge") ? "secondary" : "ghost"} aria-current={route.startsWith("/knowledge") ? "page" : undefined} onClick={() => navigate("/knowledge")}><BookOpen data-icon="inline-start" />知识（开发中）</Button>
		</nav>;
	}
	return null;
}
...
export function WorkspaceShell({
	...
	return <SidebarProvider style={{ "--sidebar-width": "calc(12.5rem + var(--sidebar-width-icon))", "--sidebar-width-icon": "3.25rem" } as CSSProperties}>
		...
		{logoutError && <div className="fixed right-4 bottom-4 z-50"><p role="alert" className="rounded-md border border-destructive bg-background p-3 text-sm text-destructive">{logoutError}</p></div>}
			<SidebarInset className="min-w-0">{!hideDesktopHeader && <header className="sticky top-0 z-10 hidden min-w-0 shrink-0 items-center gap-2 border-b bg-background p-4 md:flex"><SidebarTrigger className="-ml-1" /><div className="truncate text-sm font-medium">{moduleHeader}</div><div className="ml-auto flex gap-2">{view.actions}</div></header>}<header className="sticky top-0 z-10 flex min-w-0 shrink-0 items-center gap-2 border-b bg-background p-4 md:hidden"><SidebarTrigger className="-ml-1" /><div className="truncate text-sm font-medium">{moduleHeader}</div><Sheet><SheetTrigger asChild><Button variant="outline" size="sm" className="ml-2">菜单</Button></SheetTrigger><SheetContent side="left"><SheetHeader><SheetTitle>{moduleHeader}</SheetTitle></SheetHeader>{moduleList}</SheetContent></Sheet><div className="ml-auto flex gap-2">{view.actions}</div></header><main className={`mx-auto min-w-0 w-full p-6 ${operations ? "max-w-6xl" : "max-w-3xl"}`}>{!canAccessOperationsRoute(route, user) ? <Alert variant="destructive"><AlertDescription>此页面仅向管理员开放。</AlertDescription></Alert> : view.content}</main></SidebarInset>
	</SidebarProvider>;
}
