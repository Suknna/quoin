import type { UserSummary, BusinessView, PluginInspectionPlan } from '../../api/generated/types'
...
export interface MockState {
  ...
  commands: Map<string, { commandType: string; digest: string; status: number; body: unknown }>
  sequence: number
}
...
function baseState(scenario: MockScenario): MockState {
  ...
  const checkoutView: BusinessView = { viewKey: 'checkout', displayName: '结算', description: '结算业务范围说明', scope: { connectionName: 'thanos-primary', labelConditions: { service: 'checkout' } }, rowVersion: 3, createdAt: now, updatedAt: now }
  const catalogView: BusinessView = { viewKey: 'catalog', displayName: '目录服务', description: '不限定来源接入的示例视图', scope: { labelConditions: { service: 'catalog' } }, rowVersion: 1, createdAt: now, updatedAt: now }
  // Standalone plugin inspection plans select range via scope, independent of any business declaration.
  const latencyPlan: PluginInspectionPlan = { planKey: 'checkout-latency-watch', displayName: '结算延迟观测', enabled: true, connectionName: 'thanos-primary', pluginId: 'thanos', templateId: 'promql-check', templateVersion: '1', params: { query: 'histogram_quantile(0.95, checkout_latency)' }, scope: { kind: 'businessView', businessViewKey: 'checkout' }, cron: '*/5 * * * *', timezone: 'Asia/Shanghai', rowVersion: 2, createdAt: now, updatedAt: now }
  const clusterPlan: PluginInspectionPlan = { planKey: 'kubernetes-prod-overview', displayName: '生产集群概览', enabled: false, connectionName: 'kubernetes-prod', pluginId: 'kubernetes', templateId: 'cluster-overview', params: {}, scope: { kind: 'integration' }, cron: null, timezone: 'Asia/Shanghai', rowVersion: 1, createdAt: now, updatedAt: now }
  // A seeded active run exercises cancel semantics on a plan-scoped (non-legacy) Run.
  const activeRun: InspectionRunDetail = { id: 'inspection-run-2', planKey: latencyPlan.planKey, connectionName: latencyPlan.connectionName, state: 'Running', rowVersion: 1, triggerKind: 'schedule', scheduledFor: now, createdAt: now, checks: [], reportCount: 0, analysisActive: false }
