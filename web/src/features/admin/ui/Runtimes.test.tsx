...
describe("Runtimes", () => {
 ...
 it("confirms replacement registration and uses the expected row-version fence", async () => {
  const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ plinth: runtime }) }); vi.stubGlobal("fetch", fetchMock);
 ...
 it("labels an unregistered slot as first registration", async () => {
  const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ plinth: { ...runtime, state: "unregistered" } }) }); vi.stubGlobal("fetch", fetchMock);
  render(<Runtimes suspended={false} />);
 ...
 it("renders exactly the plinth slot", async () => {
  ...
  const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ plinth: { ...runtime, slot: "plinth", state: "unregistered" } }) }); vi.stubGlobal("fetch", fetchMock);
  ...
  expect(view.container.querySelectorAll("tbody tr")[0].textContent).toContain("plinth");
 });
 it("drops a malformed slot projection instead of rendering an unusable registration row", async () => {
  const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ plinth: { slot: "plinth" } }) }); vi.stubGlobal("fetch", fetchMock);
  const view = render(<Runtimes suspended={false} />);
  await waitFor(() => expect(view.container.querySelectorAll("tbody tr")).toHaveLength(0));
 });
 it("guards slot projections before registration actions", () => {
  const valid = { slot: "lintel", state: "unregistered", currentGeneration: 0, rowVersion: 1, connected: false };
  expect(isRuntimeSlotView(valid)).toBe(true);
  for (const malformed of [undefined, null, {}, { slot: "lintel" }, { ...valid, rowVersion: 0 }, { ...valid, state: "unknown" }, { ...valid, slot: "gateway" }]) {
   expect(isRuntimeSlotView(malformed)).toBe(false);
  }
 });
 it("clears a revealed secret explicitly", async () => {
  const fetchMock = vi.fn().mockImplementation((path: string) => path === "/api/v1/runtime" ? Promise.resolve({ ok: true, json: async () => ({ plinth: runtime }) }) : path.includes("prepare") ? Promise.resolve({ ok: true, json: async () => ({ registrationTokenAvailable: true, registrationTokenHandle: "x".repeat(32) }) }) : Promise.resolve({ ok: true, json: async () => ({ registrationToken: "secret" }) })); vi.stubGlobal("fetch", fetchMock);
  render(<Runtimes suspended={false} />);
  fireEvent.click(await screen.findByRole("button", { name: "准备替代注册" }));
  fireEvent.click(screen.getByRole("button", { name: "确认" }));
 ...
 it("does not restore an asynchronously revealed secret after suspension", async () => {
  ...
  const fetchMock = vi.fn().mockImplementation((path: string) => path === "/api/v1/runtime" ? Promise.resolve({ ok: true, json: async () => ({ plinth: runtime }) }) : path.includes("prepare") ? Promise.resolve({ ok: true, json: async () => ({ registrationTokenAvailable: true, registrationTokenHandle: "x".repeat(32) }) }) : new Promise(resolve => { reveal = resolve; })); vi.stubGlobal("fetch", fetchMock);
  const view = render(<Runtimes suspended={false} />);
  await screen.findByRole("button", { name: "准备替代注册" });
  fireEvent.click(screen.getByRole("button", { name: "准备替代注册" }));
  fireEvent.click(screen.getByRole("button", { name: "确认" }));
