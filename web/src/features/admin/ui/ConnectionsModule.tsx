...
function toInput(type: EditableConnectionType, fields: Record<string, string>): ConnectionInput {
	...
	if (type === "model_provider") return {
		type, baseUrl: fields.baseUrl, apiKey: fields.apiKey, chatModelId: fields.chatModelId,
		...(fields.embeddingModelId ? { embeddingModelId: fields.embeddingModelId } : {}),
		...
		async function changeEnabled() {
			...
			try {
				if (confirmation === "disable") onUpdate(await workbenchApi.disableConnection(selected.name, selected.rowVersion));
					...
					const qualified = results.filter(result => result.connectionType === selected.type && result.outcome === "passed" && result.connectionRevisionId === selected.currentRevisionId && result.credentialGenerationId === selected.currentCredentialGenerationId).sort((left, right) => right.finishedAt.localeCompare(left.finishedAt))[0];
						...
						if (!qualified) { setError(`${selected.type === "model_provider" ? "模型提供方" : "指标连接"}必须使用当前 revision 和凭据 generation 的已通过探测结果启用。`); return; }
...
export function useModelProviderModule({ user, route, navigate, suspended }: WorkspaceModuleProps): WorkspaceModuleView {
	...
	useEffect(() => { void load(); }, []); useEffect(() => { if (!selectedName || isNew) { detailController.current?.abort(); setSelected(undefined); setDetailLoading(false); return; } try { void chooseByName(decodeURIComponent(selectedName)); } catch { setError("连接地址无效。"); } }, [route]); useEffect(() => { if (suspended) return; const timer = window.setInterval(() => void load(), 15_000); return () => window.clearInterval(timer); }, [suspended]); useEffect(() => () => detailController.current?.abort(), []);
		const choose = (connection: ConnectionSummaryView) => navigate(`${suffix}/${encodeURIComponent(connection.name)}`); const update = (connection: ConnectionSummaryView) => { setConnections(items => items.map(item => item.name === connection.name ? connection : item)); void chooseByName(connection.name); };
		const list = <div className="px-0"><div className="border-b px-4 py-2 text-xs font-medium text-muted-foreground">模型提供方</div>{connections.map(connection => <button type="button" key={connection.name} onClick={() => choose(connection)} className="flex w-full min-w-0 flex-col items-start gap-2 border-b p-4 text-left text-sm leading-tight hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"><div className="flex w-full min-w-0 items-center gap-2"><span className="min-w-0 truncate font-medium">{connection.name}</span><span className="ml-auto shrink-0 text-xs">{connection.enabled ? "已启用" : "未启用"}</span></div><span className="line-clamp-2 min-w-0 w-full text-xs whitespace-break-spaces text-muted-foreground">{connection.revalidationRequired ? "需要重新验证" : `版本 ${connection.rowVersion}`}</span></button>)}{!loading && !error && !connections.length && <div className="p-4 text-sm text-muted-foreground">尚无可管理模型提供方。</div>}</div>;
		return { title: "模型提供方", list, actions: <Button variant="outline" size="sm" onClick={() => navigate(`${suffix}/new`)} disabled={readOnly}><Plus /> 新建模型提供方</Button>, content: <>{maintenance && <div className="mb-4 rounded-md border border-amber-500 bg-amber-50 p-3 text-sm dark:bg-amber-950">维护模式已启用：所有写入操作已阻止。</div>}{error && <div className="mb-4"><ErrorMessage>{error}</ErrorMessage></div>}{loading || detailLoading ? <div className="flex flex-col gap-3" role="status" aria-label="正在读取模型提供方服务"><Skeleton className="h-6 w-1/4" /><Skeleton className="h-24 w-full" /><Skeleton className="h-4 w-2/3" /></div> : error ? null : selected ? <ConnectionDetail key={selected.name} selected={selected} onUpdate={update} onRefresh={() => void chooseByName(selected.name)} readOnly={readOnly} suspended={suspended} /> : <ModelProviderEditor onCreated={connection => { setConnections(items => [...items, connection]); choose(connection); }} readOnly={readOnly} suspended={suspended} />}</> };
