...
function collectContacts(draft: ContactDraft): AdminContactInput[] {
	...
	if (draft.email.trim()) contacts.push({ channel: "email", target: draft.email.trim() });
	if (draft.sms.trim()) contacts.push({ channel: "sms", target: draft.sms.trim() });
	return contacts;
}
	...
	return (
		<section className="space-y-4">
			<div className="flex justify-between">
				...
				<Button disabled={suspended} onClick={() => { setNote(""); setCreating(true); }}>新建操作员</Button>
			</div>
			{error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
			{note && <Alert><AlertDescription role="status">{note}</AlertDescription></Alert>}
			<Table>
				<TableHeader>
					<TableRow>
					...
					{items.map(user => (
						<TableRow key={user.id}>
							...
							<TableCell className="space-x-2">
								<Badge variant={user.enabled ? "default" : "secondary"}>{user.enabled ? "已启用" : "已停用"}</Badge>
								{user.initialized !== undefined && (
									<Badge variant={user.initialized ? "outline" : "secondary"}>{user.initialized ? "已初始化" : "未初始化"}</Badge>
								)}
							</TableCell>
							<TableCell className="space-x-2">
								{user.role === "operator" && (
								...
								{user.role === "operator" && (
									...
									<Button size="sm" variant="outline" disabled={suspended} onClick={() => setResetting(user)}>重置密码</Button>
								)}
								<ConfirmAction
									title="撤销该用户的所有会话？"
...
function UserFields({ draft, setDraft }: { draft: { username: string; displayName: string; password: string } & ContactDraft; setDraft: React.Dispatch<React.SetStateAction<{ username: string; displayName: string; password: string } & ContactDraft>> }) {
	return (
		<>
			<Field>
				<FieldLabel htmlFor="create-username">用户名</FieldLabel>
				<Input id="create-username" value={draft.username} onChange={event => setDraft(value => ({ ...value, username: event.target.value }))} />
			</Field>
			<Field>
				<FieldLabel htmlFor="create-display-name">显示名</FieldLabel>
				<Input id="create-display-name" value={draft.displayName} onChange={event => setDraft(value => ({ ...value, displayName: event.target.value }))} />
			</Field>
			<Field>
				<FieldLabel htmlFor="create-password">临时密码</FieldLabel>
				<Input id="create-password" type="password" autoComplete="new-password" value={draft.password} onChange={event => setDraft(value => ({ ...value, password: event.target.value }))} />
				<FieldDescription>至少 15 个字符。</FieldDescription>
			</Field>
			<Field>
				<FieldLabel htmlFor="create-email-target">邮箱收码目标</FieldLabel>
				<Input id="create-email-target" type="email" placeholder="name@example.com" value={draft.email} onChange={event => setDraft(value => ({ ...value, email: event.target.value }))} />
				<FieldDescription>邮箱与短信至少配置一种；操作员初始化时须验证其中之一，之后不能自行更换。</FieldDescription>
			</Field>
			<Field>
				<FieldLabel htmlFor="create-sms-target">短信收码目标</FieldLabel>
				<Input id="create-sms-target" type="tel" placeholder="+8613800000000" value={draft.sms} onChange={event => setDraft(value => ({ ...value, sms: event.target.value }))} />
			</Field>
		</>
	);
}
