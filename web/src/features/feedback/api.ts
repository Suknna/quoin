...
export async function appendFeedback(
	...
	const response = await fetch("/api/v1/knowledge/feedback", {
		...
		body: JSON.stringify({
			clientCommandId: feedbackCommandId(),
...
export async function fetchFeedback(
	target: FeedbackTarget,
): Promise<FeedbackTimeline> {
	const query = `targetType=${encodeURIComponent(target.type)}&targetId=${encodeURIComponent(target.id)}&limit=50`;
	const response = await fetch(`/api/v1/knowledge/feedback?${query}`, {
		credentials: "include",
	});
	if (!response.ok) throw new Error(await problemMessage(response));
