...
const correlationEvents = [
	{ id: "e2", correlationId: "corr-1", actorType: "user", actorId: "u1", action: "user.created", outcome: "success", phase: "outcome", domainRefType: "user", domainRefId: "u9", createdAt: "2026-09-01T08:00:00Z" },
	{ id: "e1", correlationId: "corr-1", actorType: "service", actorId: "executor", action: "command.attempt", outcome: "success", phase: "execution", attemptId: "att-1", createdAt: "2026-09-01T07:59:59Z" },
];

const listedEvents = [
	...correlationEvents,
	{ id: "e0", actorType: "user", actorId: "u2", action: "legacy.action", outcome: "rejected", createdAt: "2026-08-01T00:00:00Z" },
];
...
describe("audit admin screen", () => {
	it("lists events, marks legacy history without correlation, and shows the correlated timeline in order", async () => {
		...
		const timeline = within(dialog).getAllByText(/user\.created|command\.attempt/);
	...
	it("passes filters as query parameters and pages by opaque cursor", async () => {
		const recent = { id: "e2", actorType: "user", actorId: "u1", action: "user.created", outcome: "success", createdAt: "2026-09-01T08:00:00Z" };
		const older = { id: "e9", actorType: "user", actorId: "u1", action: "user.disabled", outcome: "success", createdAt: "2026-01-01T08:00:00Z" };
		const fetchMock = stubAuditFetch((url) =>
		...
		fireEvent.change(screen.getByLabelText("关联 ID"), { target: { value: "corr-9" } });
		fireEvent.click(screen.getByRole("button", { name: "应用筛选" }));

		await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(3));
		const filtered = new URL(String(fetchMock.mock.calls[2][0]), "https://quoin.invalid").searchParams;
		expect(new Date(filtered.get("since")!).getTime()).toBe(new Date(2026, 8, 1).getTime());
	...
	it("previews the retention impact, requires confirmation, and applies the change", async () => {
		...
		expect(within(dialog).getByRole("button", { name: "预览影响" })).toBeDisabled();
		...
		expect(within(dialog).getByText(/预计到期事件：\s*3\s*条/)).toBeInTheDocument();
	...
	it("reports a settings conflict instead of pretending the change succeeded", async () => {
		...
		fireEvent.click(await screen.findByRole("button", { name: "修改保留期" }));
		const dialog = screen.getByRole("dialog");
		fireEvent.change(within(dialog).getByLabelText("新保留期（自然月）"), { target: { value: "12" } });
		fireEvent.click(within(dialog).getByRole("button", { name: "预览影响" }));
		await waitFor(() =>
			expect(within(dialog).getByRole("button", { name: "确认调整" })).toBeEnabled(),
		);
		fireEvent.click(within(dialog).getByRole("button", { name: "确认调整" }));
		expect(await within(dialog).findByText(/已被其他管理员修改/)).toBeInTheDocument();
	...
	it("surfaces server problems instead of rendering invented events", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => jsonResponse({ message: "数据库暂时不可用。" }, false, 503)),
		);
		render(<AuditPage suspended={false} />);
