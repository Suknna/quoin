...
function stubApi(
	...
	vi.stubGlobal(
		...
		vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (method === "GET" && url === "/api/v1/business-views")
				return Response.json({ items: [checkoutView] });
			if (method === "GET" && url === "/api/v1/business-views/checkout")
				return Response.json(checkoutView);
			if (method === "GET" && url === "/api/v1/connections?limit=100")
...
function ModuleHarness({
	...
	const props: WorkspaceModuleProps = {
		user: { role: "admin" } as WorkspaceModuleProps["user"],
		route,
		navigate,
		suspended,
		openEvidence: vi.fn(),
	...
	return (
		<div>
			<section aria-label="模块操作">{view.actions}</section>
...
describe("business views module", () => {
	...
	it("shows view scope, label conditions and routes to the editor", async () => {
		...
		// 详情渲染在右侧抽屉（portal 到 body），不在模块内容容器内。
		expect(
			await screen.findByRole("heading", { name: "结算" }),
		).toBeInTheDocument();
		expect(screen.getByText("结算业务范围说明")).toBeInTheDocument();
		// 概览列表仍渲染在抽屉下方，接入名/标识在列表与抽屉中各出现一次。
		expect(screen.getAllByText("thanos-primary").length).toBeGreaterThan(0);
		expect(screen.getAllByText("checkout").length).toBeGreaterThan(0);
		expect(screen.getByText("service")).toBeInTheDocument();
		expect(screen.getByText("更新时间")).toBeInTheDocument();
		fireEvent.click(screen.getByRole("button", { name: "编辑" }));
		expect(navigate).toHaveBeenCalledWith(
			"/business-views?view=checkout&edit=1",
		);
	});
	...
	it("offers an inspection entry carrying the view scope for preselection", async () => {
		...
		render(<ModuleHarness route="/business-views?view=checkout" />);
		fireEvent.click(
			await screen.findByRole("button", { name: /按此视图巡检/ }),
		);
		expect(navigate).toHaveBeenCalledWith(
			"/inspections?businessViewKey=checkout&connectionName=thanos-primary",
		);
	});

	it("omits the inspection connection parameter when the view scopes all sources", async () => {
		stubApi({
			"GET /api/v1/business-views/checkout": () =>
				Response.json({
					...checkoutView,
		...
		expect(navigate).toHaveBeenCalledWith(
			"/inspections?businessViewKey=checkout",
		);
	});

	it("creates a view through the shared draft and routes to its detail", async () => {
		stubApi({
			"POST /api/v1/business-views": () =>
				Response.json({ ...checkoutView, rowVersion: 1 }),
		});
		render(<ModuleHarness route="/business-views/new" />);
		fireEvent.change(await screen.findByLabelText("视图标识"), {
			target: { value: "checkout" },
		});
		fireEvent.change(screen.getByLabelText("显示名称"), {
			target: { value: "结算" },
		});
		fireEvent.change(screen.getByLabelText("业务说明"), {
			target: { value: "结算业务范围说明" },
		});
		...
		fireEvent.click(
			await screen.findByRole("option", { name: "thanos-primary" }),
		);
	...
	it("updates an existing view with optimistic row-version fencing and a frozen key", async () => {
		stubApi({
			"PUT /api/v1/business-views/checkout": () =>
				Response.json({ ...checkoutView, rowVersion: 4 }),
		});
	...
	it("surfaces save conflicts without destroying the draft", async () => {
		...
		fireEvent.click(screen.getByRole("button", { name: "保存业务视图" }));
		expect(
			await screen.findByText("视图已被其他人更新，请刷新后重试。"),
		).toBeInTheDocument();
		expect((screen.getByLabelText("显示名称") as HTMLInputElement).value).toBe(
			"结算系统",
		);
	...
	it("stays usable when the metrics connection list is unavailable", async () => {
		stubApi({
			"GET /api/v1/connections?limit=100": () =>
				Response.json({ message: "无法读取连接。" }, { status: 503 }),
		...
		await waitFor(() =>
			expect(screen.getByText(/候选来源为全部接入/)).toBeInTheDocument(),
		);
		fireEvent.click(screen.getByRole("button", { name: "保存业务视图" }));
		await waitFor(() => expect(posted).toHaveLength(1));
		expect(requestBody(0).scope).toEqual({ labelConditions: {} });
		await waitFor(() =>
			expect(navigate).toHaveBeenCalledWith("/business-views?view=all"),
		);
	});
