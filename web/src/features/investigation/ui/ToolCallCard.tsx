...
function resultSummary(call: ToolCallItem): string {
	if (call.status === "failed" && call.errorDetail)
		return `失败原因：${call.errorDetail}`;
	...
	if (Array.isArray(call.result))
		return `返回数组 · ${call.result.length} 项。`;
	if (typeof call.result === "object")
		return `返回对象 · ${Object.keys(call.result).length} 个字段。`;
	...
	return (
		<Collapsible className="rounded-lg border">
			<CollapsibleTrigger asChild>
				<Button
					variant="ghost"
					className="group h-auto w-full justify-start gap-2 whitespace-normal py-2 text-left"
				>
			...
			<CollapsibleContent>
				<div className="flex flex-col gap-3 border-t px-3 py-2 text-sm">
					<PropertyList
						entries={[
							{ label: "执行模式", value: call.executionMode },
							{ label: "失败模式", value: call.failureMode },
							...(call.startedAt
								? [{ label: "开始", value: formatDateTime(call.startedAt) }]
								: []),
							...(call.endedAt
								? [{ label: "结束", value: formatDateTime(call.endedAt) }]
								: []),
							...(call.errorDetail
								? [
										{
											label: "错误",
											value: (
												<span className="break-words">{call.errorDetail}</span>
											),
										},
									]
								: []),
					...
					<section className="flex flex-col gap-2">
						...
						<StructuredData
							...
							raw={JSON.stringify(call.arguments, null, 2)}
						/>
					</section>
					{call.result !== undefined && call.result !== null ? (
						<section className="flex flex-col gap-2">
							...
							<StructuredData
								value={call.result}
								raw={JSON.stringify(call.result, null, 2)}
							/>
						</section>
					) : (
