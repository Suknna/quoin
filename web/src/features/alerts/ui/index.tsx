...
import { cn } from "cn";
import { Activity, AlertTriangle, Bot, CheckCircle2, ChevronDown, CircleDot, Clock3, FileText, RefreshCw } from "lucide-react";
...
import { fetchAlerts, fetchObservations, fetchOccurrence, type AlertOccurrenceSummary, type AttributionDiagnostic, type ObservationSummary, type ViewAttributionDiagnostic } from "@/features/alerts/api";
...
function listRoute(view: "current" | "history", viewKey: string, id?: string) { const query = new URLSearchParams({ view }); if (viewKey) query.set("viewKey", viewKey); if (id) query.set("id", id); return `/alerts/list?${query}`; }
...
export function useAlertsModule(props: WorkspaceModuleProps): WorkspaceModuleView {
  ...
  useEffect(() => { if (path === "/alerts") props.navigate(listRoute(view, viewKey, selectedId ?? undefined)); }, [path, props, selectedId, view, viewKey]);
  ...
  return { title: "运维中心", list: null, content: <AlertList {...props} view={view} viewKey={viewKey} legacySystem={legacySystem} selectedId={selectedId} /> };
}
...
function viewCandidates(value: string): string {
  try {
    ...
    const keys = parsed.map((entry) => (entry && typeof entry === "object" && "viewKey" in entry && typeof entry.viewKey === "string" ? entry.viewKey : "")).filter(Boolean);
    return keys.length ? keys.join(", ") : value;
  } catch { return value || "无"; }
}

function ViewAttributionNotice({ attribution }: { attribution: ViewAttributionDiagnostic }) {
  if (attribution.status === "attributed") return null;
  const ambiguous = attribution.status === "ambiguous";
  return <Alert variant={ambiguous ? "destructive" : "default"} aria-label={ambiguous ? "归属歧义诊断" : "未归属诊断"}>
    <AlertTriangle />
    <AlertTitle>{ambiguous ? "归属歧义" : "未归属"}</AlertTitle>
    <AlertDescription>
      <p>{ambiguous ? "该告警首次接收时匹配多个业务视图，未归属到任一视图；请调整视图的告警源或标签条件后，由新告警重新归属。" : "该告警首次接收时未匹配任何业务视图。"}</p>
      <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1">
        <dt>原因</dt><dd className="break-words font-mono">{attributionReason(attribution.reasonJson)}</dd>
        {ambiguous && <Fragment><dt>候选视图</dt><dd className="break-words font-mono">{viewCandidates(attribution.candidatesJson)}</dd></Fragment>}
      </dl>
      <p>以上为告警首次接收时冻结的归属证据；历史记录不会按当前视图配置重新计算。</p>
    </AlertDescription>
  </Alert>;
}

function AlertList({ view, viewKey, legacySystem, selectedId, navigate, suspended, openEvidence }: WorkspaceModuleProps & { view: "current" | "history"; viewKey: string; legacySystem: string; selectedId: string | null }) {
  const [views, setViews] = useState<{ viewKey: string; displayName: string }[]>([]);
  ...
  const live = useLiveAlerts(view === "history" ? "Resolved" : "Firing", filterKind === "viewKey" ? { viewKey: filter } : { businessSystemKey: filter }, !suspended);
  const { setAtTop } = live;
  useEffect(() => { if (!suspended) listBusinessViews().then((items) => setViews(items.map((item) => ({ viewKey: item.viewKey, displayName: item.displayName })))).catch(() => undefined); }, [suspended]);
  useEffect(() => {
  ...
  const refreshCounts = useCallback(async (requestGeneration = countGenerationRef.current) => {
    ...
    try {
      ...
      const [firing, resolved] = await Promise.all([
        fetchAlerts("Firing", filterKind === "businessSystemKey" ? filter : "", filterKind === "viewKey" ? filter : ""),
        fetchAlerts("Resolved", filterKind === "businessSystemKey" ? filter : "", filterKind === "viewKey" ? filter : ""),
      ]);
      if (requestGeneration === countGenerationRef.current && !suspended) setCounts({ key: filter, firing: firing.items.length, resolved: resolved.items.length });
  ...
  const listItems: EntityListItem[] = filteredItems.map((item) => {
    ...
    const attributedView = item.source === "alertmanager" && item.viewAttribution?.status === "attributed" ? item.viewAttribution.viewName || item.viewAttribution.viewKey : undefined;
    const diagnosticLabel = item.source === "alertmanager"
      ? (item.viewAttribution?.status === "ambiguous" ? "归属歧义" : item.viewAttribution?.status === "unattributed" ? "未归属" : !item.viewAttribution && item.attribution?.status === "conflict" ? "归属冲突" : !item.viewAttribution && item.attribution?.status === "unattributed" ? "未归属" : undefined)
      : undefined;
    return {
      id: item.id,
      title: item.labels.alertname ?? item.id,
      subtitle: item.annotations?.summary ?? item.annotations?.description ?? attributedView ?? item.businessSystemKey ?? diagnosticLabel ?? "未提供摘要",
  ...
  const controls = <div className="flex flex-wrap items-center justify-between gap-3"><Tabs value={view} onValueChange={(next) => navigate(listRoute(next as "current" | "history", viewKey, selectedId ?? undefined))}><TabsList><TabsTrigger value="current">当前告警{counts.key === countKey && counts.firing !== undefined && <Badge variant="secondary" className="ml-1 tabular-nums">{counts.firing}</Badge>}</TabsTrigger><TabsTrigger value="history">历史告警{counts.key === countKey && counts.resolved !== undefined && <Badge variant="secondary" className="ml-1 tabular-nums">{counts.resolved}</Badge>}</TabsTrigger></TabsList></Tabs><div className="flex flex-1 flex-wrap justify-end gap-3"><Select value={viewKey || "__all__"} onValueChange={(next) => navigate(listRoute(view, next === "__all__" ? "" : next, selectedId ?? undefined))}><SelectTrigger className="w-full sm:w-52" aria-label="按业务视图过滤"><SelectValue placeholder="全部业务视图" /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="__all__">全部业务视图</SelectItem>{views.map((item) => <SelectItem key={item.viewKey} value={item.viewKey}>{item.displayName}</SelectItem>)}</SelectGroup></SelectContent></Select><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索已加载告警" className="w-full sm:max-w-xs" aria-label="搜索已加载告警" /></div></div>;
  return <section className="mx-auto flex w-full max-w-6xl flex-col gap-6"><div><h1 className="text-2xl font-semibold tracking-tight">{view === "current" ? "当前告警" : "告警历史"}</h1><p className="mt-1 text-sm text-muted-foreground">查看上游告警与平台内部组件故障的真实记录；归属来自业务视图的首次接收判断。</p></div>{live.pendingNew > 0 && <Button variant="secondary" size="sm" onClick={live.mergePending}>显示 {live.pendingNew} 条新告警</Button>}<EntityList items={listItems} columns={["media", "title", "subtitle", "status", "time"]} selectedId={selectedId} onSelect={(item) => navigate(listRoute(view, viewKey, item.id))} loading={live.loading} error={live.error} onRetry={live.refresh} controls={controls} emptyTitle={emptyTitle} emptyDescription={emptyDescription} /><AlertDetailSheet id={selectedId} onClose={() => navigate(listRoute(view, viewKey))} suspended={suspended} openEvidence={openEvidence} /></section>;
}
