...
export function EvidenceReader({
	...
	return <EvidenceReadingSession key={id} id={id} user={user} />;
}

function EvidenceReadingSession({
	id,
	user,
}: {
	id: string;
			...
			.then(async (detail) => {
				...
				if (detail.body.kind === "artifact") {
					...
					if (!metadata.bodyExpired) {
						const response = await fetch(artifactDownloadURL(metadata.id), {
							credentials: "include",
	...
	return (
		<section className="flex min-h-0 flex-1 flex-col p-6">
			<header className="flex items-start justify-between gap-3">
				<div>
					<h1 className="text-xl font-semibold">证据阅读</h1>
			...
			<div className="mt-4 flex flex-wrap gap-2">
				<Badge
					variant={
						evidence.integrity === "complete" ? "secondary" : "destructive"
					}
				>
					{evidence.integrity === "complete" ? "完整" : "不完整"}
				</Badge>
				{evidence.connections.map((item) => (
					<Badge variant="outline" key={item.key}>
						{item.key}
					</Badge>
				))}
			</div>
			{(warnings.length > 0 || errors.length > 0) && (
				...
				>
					<AlertDescription>
						{warnings.map(String).join("；")} {errors.map(String).join("；")}
					</AlertDescription>
				</Alert>
			)}
			<Accordion
				className="mt-4"
				type="multiple"
				defaultValue={["parameters", "content"]}
			>
				<AccordionItem value="parameters">
					<AccordionTrigger>来源与查询参数</AccordionTrigger>
					<AccordionContent>
						<p className="text-sm">来源：{evidence.producer.kind}</p>
						<pre className="mt-2 overflow-x-auto rounded bg-muted p-3 text-xs">
							{evidenceParamsText(evidence.params)}
						</pre>
					</AccordionContent>
				</AccordionItem>
				<AccordionItem value="content">
					<AccordionTrigger>证据内容</AccordionTrigger>
					<AccordionContent>
						{evidence.body.kind === "inline_json" ? (
							<InlineEvidence value={evidence.body.value} />
						) : (
							<ArtifactContent
								artifact={artifact}
								content={content}
		...
		<div className="space-y-3">
			...
			<Accordion type="single" collapsible>
				<AccordionItem value="raw">
					<AccordionTrigger>原始证据正文</AccordionTrigger>
					<AccordionContent>
...
function ArtifactContent({
	...
	return (
		<div className="space-y-3">
			...
			{artifact.sensitive && user.role !== "admin" ? (
				<Alert variant="destructive">
					<AlertDescription>
						此敏感产物由服务端权限策略控制；若无法显示内容，请联系管理员授权。
					</AlertDescription>
				</Alert>
			) : (
