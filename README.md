...
Quoin 通过插件扩展平台接入能力，将不同来源的告警、指标查询、资源观测与巡检能力接入统一工作台。插件体系 v2（ADR-0011）按网关边界划分两类能力：入向事件源（EventSource，在 Stele 网关解析归一化）与出向工具（泛型类型安全工具，业务逻辑在 Quoin、传输在 Stele）。

| 插件 | 状态 | 能力或方向 |
| --- | --- | --- |
| Prometheus | 当前可用 | 指标查询、采集目标观测与巡检采证 |
| Thanos | 当前可用 | Prometheus 兼容指标查询、目标观测与巡检采证 |
| Alertmanager | 当前可用 | 告警事件接入（Stele 网关认证归一化入队） |

当前插件随 Quoin 构建发布（接口 + 注册表 + 空白导入的编译期装配，无动态加载）。扩展机制见[插件开发文档](docs/plugin-development.md)。
...
业务服务 / exporters ──→ Prometheus / Thanos
                                  ▲
                       指标查询与巡检采证（Stele 网关执行）
                                  │
Alertmanager ──→ Stele ──→ Quoin ←──→ Plinth
（事件入队即 ACK）  （调度中枢）   （模型推理，插件零感知）
                            │
                       Web 工作台
```

三组件职责（ADR-0011）：Quoin 是调度中枢（插件注册、工具目录聚合、权限/审计/派发、业务库唯一写者）；Stele 是外部网关（所有外部平台交互的唯一出入口：入向事件队列化、出向凭证注入/限流/执行，只做传输与协议转换）；Plinth 是推理沙箱（协议只有「任务 + 工具目录 → 文本或 tool_call」）。
