...
登录与首次改密共用右侧品牌展示区：石墨色背景、产品定位文案和证据关联示意，不读取业务数据或展示运行状态。每次挂载（首次进入或刷新）播放约 1.65 秒的单次入场动画，标题与节点依次淡入、连线逐步呈现，不循环；系统启用“减少动态效果”时直接显示完整内容，左侧认证表单始终可立即操作。小于 1024 CSS px 时隐藏展示区，保留品牌与认证表单；可用上述两个场景检查不同屏宽和认证状态。
