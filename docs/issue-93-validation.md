...
本记录描述实际验证范围，不替代 ADR-0001 的目标架构，也不表示 Issue #93 已全部完成。
...
- Kubernetes 专用 Lintel 恢复验收尚未完整移植其离线凭据签发、存储隔离证明、等待认证和最终收据流程；对应专用验证入口必须明确返回 `NOT_RUN`，不得输出通过的事实。本项是 Issue #93 的剩余工作，Issue 保持开放。
- 新增 Kubernetes 专用 restore-isolation 处理器有聚焦测试，但未在本次真实集群中完成其完整破坏性恢复场景。
