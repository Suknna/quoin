...
开发命令：`pnpm --dir web dev`。

模板来源已并入唯一应用根，生产产物为 `web/dist`。不改旧 web/index.html、src 入口、旧 Vite 配置、Go embed dist 或实验部署。新 CSS 使用 Tailwind `source(none)` 并仅扫描 templates，不导入旧 CSS。
...
- `components.json` 使用 new-york-v4、rsc=false、aliases 指向 src。工具 JSON 与已有 package/tsconfig 是工具契约例外，自有台账采用 YAML。
...
- `pnpm --dir web build`：通过，包括独立模板 TypeScript 检查。
