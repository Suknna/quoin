...
```sh
NODE_EXTRA_CA_CERTS="$PWD/.local-lab/config/kubernetes/tls/quoin-lab-ca.crt" pnpm --dir web dev
pnpm --dir web build
pnpm --dir web test
```
...
独立 root 为 `web/src`，构建输出 `.artifacts/new-workbench/dist`；不覆盖旧 `internal/gen/web/dist`，不发布镜像。
