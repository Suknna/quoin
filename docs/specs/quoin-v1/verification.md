...
- **VERIFY-VALIDATION-003 —** Deployment Acceptance **MUST** 对 helper、manifest/results/conflicts/receipt、Connection Probe、Config Verification、Browser/noVNC/cleanup、8 小时时间闭包、retention 与 `recover-lintel` 构造正常、失败、重放、并发、漂移、取消、崩溃和新 boot 对抗用例；只有零 verifier conflict 且全部 applicable required item PASSED 才可 PASSED。（来源：Issue #21 Q21.32–Q21.40）
