...
> **验证线移除（2026-09）：** 验证驱动（quoin-verify/quoin-faultfs/quoin-deploy/qualify/aggregate）、CI 工作流与站点级 Deployment Acceptance 空壳（API 声明、verification_invocation_* 表、deployment-verification/verification-result 契约）已全部移除。本文保留为历史设计解读；当前可执行验证是 go test/vet、产品内 Config Verification Run 与发布物签名验证。
