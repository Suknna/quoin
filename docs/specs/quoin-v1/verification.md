...
- **VERIFY-VALIDATION-006 —** Issue #93 实施完成后，Contract Gate 与 Release Qualification MUST 新增或替换为可观察行为验证：相同 Proto 契约指纹而发布版本不同的 Quoin/Plinth/Lintel/Stele 通信成功；相同发布版本而指纹缺失、无效或不同则在注册、连接、重连与 Stele RPC 明确拒绝，且不得绕过 token、轮换、吊销、TLS 或 epoch；修改任一权威 proto 文件改变统一指纹并选择五个应用重建，OpenAPI 变化选择 Quoin 与前端重建，局部实现变化可局部构建且固定第三方 Caddy 不重建。入口 Caddy 的真实六服务部署还 MUST 覆盖首页、深层路由刷新、登录、API、SSE、noVNC WebSocket、同源 Cookie/CSRF/Origin 和静态响应安全头，并证明 API 失败不返回 SPA fallback。普通 Kubernetes YAML 和简单 Compose 各自必须真实启动该拓扑；该条款是未来验收要求，不声称现有历史 Helm/Compose 证据已覆盖新架构。（来源：[Issue #93](https://github.com/Suknna/quoin/issues/93)、ADR-0001）
