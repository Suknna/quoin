...
- **VERIFY-FAULT-002 —（历史）** 存储故障精确注入原语（ENOSPC、EDQUOT、EROFS、指定 fsync/rename 失败）连同其 verification-only `quoin-faultfs` 执行器已随验证驱动一并移除；本条款保留为历史解读，Release Qualification 重建前不再有 CI 执行方。（来源：Issue #21 Q21.16）
...
- **VERIFY-CLEANUP-002 —** 正常 teardown 后 invocation 拥有的 Pod、Job、Service、Secret、PVC/volume、network、container、临时文件与临时 registry 数据 **MUST** 机械证明归零；产品/helper residue **MUST** 为 FAILED，CI/集群故障导致无法判断 **MUST** 为 WARNED。（来源：Issue #21 Q21.17）
...
- **VERIFY-CONNECTION-004 —** Prometheus 与 Thanos probe **MUST** 分别经 Plinth 对已冻结的实际类型化配置执行固定 `vector(1)`；每种支持认证模式（`none`、HTTP Basic、Bearer）和既有 TLS 配置都必须走真实请求，且不得把 TLS 验证关闭伪装为认证模式。Quoin PromQL path **MUST** 由 Config Verification 单独证明。Kubernetes probe **MUST** 验证 `/version`、`/api`、`/apis` 与 effective namespace 中 pods get/list、events list、pods/log get 的 SelfSubjectAccessReview，且 **MUST NOT** 外推其它权限或全部 Business System mapping。（来源：[#97](https://github.com/Suknna/quoin/issues/97)、Issue #21 Q21.36）
...
- **VERIFY-VALIDATION-003 —** Deployment Acceptance **MUST** 对 helper、manifest/results/conflicts/receipt、Connection Probe 与 typed observation 路径的所有写入口执行对抗验证。
- **VERIFY-VALIDATION-006 —** Issue #93 实施完成后，Contract Gate 与 Release Qualification MUST 新增或替换为可观察行为验证：相同 Proto 契约指纹而发布版本不同的 Quoin/Plinth/Stele 通信成功；相同发布版本而指纹缺失、无效或不同则在注册、连接、重连与 Stele RPC 明确拒绝，且不得绕过 token、轮换、吊销、TLS 或 epoch；修改任一权威 proto 文件改变统一指纹并选择四个应用重建，OpenAPI 变化选择 Quoin 与前端重建，局部实现变化可局部构建且固定第三方 Caddy 不重建。入口 Caddy 的真实五服务部署还 MUST 覆盖首页、深层路由刷新、登录、API、SSE、同源 Cookie/CSRF/Origin 和静态响应安全头，并证明 API 失败不返回 SPA fallback。普通 Kubernetes YAML 和简单 Compose 各自必须真实启动该拓扑；该条款是未来验收要求，不声称现有历史 Helm/Compose 证据已覆盖新架构。（来源：[Issue #93](https://github.com/Suknna/quoin/issues/93)、ADR-0001）
