...
- **UI-SHELL-002 —** 全局导航固定为：运维中心、AI SRE、管理员设置。运维中心包含告警列表、故障复盘、巡检、业务视图和接入管理子页，其中巡检、业务视图与接入管理仅 Admin 可见；AI SRE 包含对话和知识子页；管理员设置只对 Admin 显示。Operator 只显示和使用告警与 AI SRE；导航可见性不放宽服务端权限：服务端必须在每次受限请求上拒绝 Operator。登录成功默认进入运维中心的告警页，不增加首页。（来源：[CONTEXT.md](../../../CONTEXT.md)、[#95](https://github.com/Suknna/quoin/issues/95)、[ADR-0004](../../adr/0004-plugin-capability-registry.md)）
...
- **UI-ROUTE-003 —** Evidence、Initial Analysis 输出、Inspection Report、Knowledge/Version、业务视图与来源观测对象使用嵌套路由铺满工作台。至少支持以下路由形态；具体 path parameter 对应 OpenAPI locator：（来源：[CONTEXT「工作台投影」](../../../CONTEXT.md#工作台投影)、[Issue #15](https://github.com/Suknna/quoin/issues/15)）
  ...
  - `/business-views?view=:viewKey`（业务视图详情/编辑层，`edit=1` 进入编辑；表单与 YAML 编辑同一对象）
  - `/integrations/:platform/:connectionId`（接入详情：验证、启用、凭据与观测资源）
  - `/business-systems/:systemKey/configs/:configVersionId`、`/business-systems/:systemKey/resources/:resourceId`（历史声明版本与 Observed Resource，只读）
...
- **UI-AUTH-002 —** Session 失效时立即卸载工作区并直接进入完整登录页面：受保护内容、未提交草稿与页面内存中的一次性秘密随之丢弃，不保留遮蔽层或同用户草稿恢复；任何 principal 重新登录都从全新工作区开始，草稿不写浏览器持久存储。（来源：[CONTEXT「工作台投影」](../../../CONTEXT.md#工作台投影)、[Issue #15](https://github.com/Suknna/quoin/issues/15)）
- **UI-AUTH-003 —（superseded by 统一认证流程）** 历史的“临时密码验证后设置新密码即建立 Session”两阶段已被服务端认证流程替代：凭据验证成功只开启短时认证流程（`__Host-quoin-flow` Cookie），登录页按 `AuthFlow.type` 推进——login 流程始终推进到二级验证，不回退任何设置步骤；初始化/首次登录需设置正式密码（页面展示实际 15–128 Unicode 规则，允许粘贴、自动填充和密码管理器）并验证收码渠道，完成步骤以流程内已验证因素（`AuthFlow.factorVerified`）为准，刷新或恢复后不因账户级渠道的历史验证状态跳过本次挑战，初始化完成返回登录页、不建立 Session；login 流程完成二级验证后才建立正常 Session 加载工作台。验证码输入展示脱敏目标并可重发；投递失败不得降级为密码单因素登录。（来源：[CONTEXT「工作台投影」](../../../CONTEXT.md#工作台投影)、[统一认证设计 §3/§4](../../authentication-design.md)）
...
- **UI-AUTH-005 —** `/audit` 是仅 Admin 可从头像菜单打开的全工作台列表，不是第七个模块；服务端必须拒绝 Operator 直接访问。Admin 可按关联 ID、操作者类型、动作、结果与时间窗（since/until）筛选并查看结构化详情（phase 与 correlation 关联视图按机器契约投影）；管理员设置内的审计分组提供保留月数设置、影响预览与最近清理状态。已删除的过期历史如实呈现保留边界，不伪装为完整链路。（来源：[CONTEXT「账号、Session 与审计投影」](../../../CONTEXT.md#账号session-与审计投影)、[审计设计 §6/§7](../../audit-design.md)）

## 5. 告警工作台

- **UI-ALERT-001 —** 统一告警页按来源投影上游 Alertmanager Occurrence 与平台故障；两者均保留真实来源和自身生命周期，平台故障不得伪造 Delivery、Occurrence 或业务归属。既有上游告警列表、详情和初步分析交互保持；接入问题继续是独立查询，不得伪装为生命周期状态。具体路由、筛选和读模型字段待 #95 实施时与机器契约同步确定。（来源：[CONTEXT「平台故障」](../../../CONTEXT.md#平台故障)、[#95](https://github.com/Suknna/quoin/issues/95)、ADR-0002）
...
- **UI-ALERT-006 —** 接入问题管理仅 Admin 可进入：未确认项在前，列表显示 kind、逻辑来源、首次/最近发生时间和重复次数；详情先用普通语言解释影响，再展示 Delivery、item index、labels/fingerprint、截断数量与不可变事件历史。Admin 可原位“标记已处理”，完成后仍可用已处理筛选查看。同 signature 再发生时出现新的待处理对象。Operator 不得进入管理页面或直接读取其 API，只能在告警页看到授权的非秘密影响提示；服务端必须强制拒绝直接访问。（来源：[CONTEXT「告警接入问题」](../../../CONTEXT.md#告警接入问题)）
...
- **UI-ADMIN-003 —（superseded by ADR-0005 唯一管理员模型；联系方式语义随 ADR-0010 调整）** 用户列表显示用户名、显示名、角色、启用状态、初始化状态和最后登录；创建只产生 Operator，联系方式为可选展示项（0..2，每渠道至多一个，不提供角色选择），详情编辑仅显示名/启用状态——角色不可变更，不存在 Operator 提升；用户管理另提供联系方式整体替换（携带 row version 前提）、重置密码与撤销全部 Session。内置 Admin 不显示禁用/降级/角色操作；联系方式变更仅影响展示，不失效 Session、不影响初始化。（来源：[CONTEXT「工作台投影」](../../../CONTEXT.md#工作台投影)、[统一认证设计 §1/§3](../../authentication-design.md)、[ADR-0010](../../adr/0010-oidc-auth-and-local-emergency.md)）
