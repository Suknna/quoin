...
- **UI-AUTH-005 —** `/audit` 是仅 Admin 可从头像菜单打开的全工作台列表，不是第七个模块；服务端必须拒绝 Operator 直接访问。Admin 可按操作者类型、动作和时间筛选并查看结构化详情。（来源：[CONTEXT「账号、Session 与审计投影」](../../../CONTEXT.md#账号session-与审计投影)）
...
6. **UI-TEST-006 权限/认证**：Operator/Admin 导航差异、受限 URL 403、Session 失效/撤销立即卸载工作区并进入完整登录、重新登录后全新工作区、临时密码两阶段和一次性秘密内存边界。（来源：[CONTEXT「工作台投影」](../../../CONTEXT.md#工作台投影)、[Issue #15](https://github.com/Suknna/quoin/issues/15)）
