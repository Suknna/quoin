...
- **ARCH-OBS-004 —** 四组件用户/Runtime 数据面与内部运维面必须使用不同固定监听器；`/livez`、`/readyz`、`/metrics` 只存在于运维监听器，网络可达性与探针时序由 [operations.md](operations.md) 的 OPS-NET-* / OPS-HEALTH-* / OPS-HELM-* 独占定义。（来源：[Issue #17](https://github.com/Suknna/quoin/issues/17) Q17.6/Q17.17）
