...
> **浏览器与 Kubernetes 插件移除（2026-09）：** 浏览器插件/Lintel 链路与 kubernetes 插件已从代码与契约中彻底移除；文中浏览器执行链与 Kubernetes 插件条款仅作历史解读。
...
> **现状与迁移（ADR-0004）：** 插件化主线已实施：接入验证并启用即获得来源级观测、`sourceRef` 授权工具与独立巡检计划，业务视图可选；`BusinessSystem` 单一声明与活动全局 Label Contract 已退出主线，声明写入与发布入口移除，既有声明、版本、Run 与 E2E 证据作为只读历史保留，禁止按新模型重写过去事实；旧业务系统定位（如 Kubernetes 绑定、businessSystem 工具参数）仅作为历史 Attempt 的冻结解释与显式迁移状态存在。接入保存访问与凭据边界；模型供应商仍是唯一由 Admin 管理的 provider 连接，不属于接入或任何业务对象。

- **ARCH-AUTH-001 —** Quoin **MUST** 是用户、配置、连接、凭据、任务、消息、Evidence、Artifact、Knowledge 与全部执行状态的唯一持久权威。Plinth、Stele 与前端都只能持有可丢弃投影，**MUST NOT** 建立可独立续跑或反向覆盖 Quoin 的第二历史。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-AUTH-006 —** React 前端是独立构建、独立发布的静态应用，经入口 Caddy 同源访问 Quoin HTTP/SSE 的可丢弃投影：可在当前 history entry 保存滚动、折叠和布局，在当前页面内存保存未提交非秘密输入，但 **MUST NOT** 持久化领域状态、秘密、通知、任务进度或恢复 checkpoint；刷新后的全部可恢复业务事实必须来自 Quoin。（来源：[Issue #15](https://github.com/Suknna/quoin/issues/15)、[frontend.md](frontend.md)）
...
- **ARCH-COMP-001 —** 用户浏览器/Operator 经入口 Caddy 以单一公共 Origin 访问前端静态页面与 Quoin 的 HTTP、SSE 与 assistant-ui message stream；Plinth 主动向 Quoin 建立出站 gRPC，Stele 只向 Quoin 提交告警 Delivery。Runtime 之间 **MUST NOT** 直连。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-COMP-003 —（ADR-0011 修订）** Plinth 是部署级唯一无状态推理沙箱（Agent Runtime）：领取固定工作类型，在每个 Attempt 的全新工作区中运行一个 worker，并由 supervisor 代为调用模型；类型化外部工具（quoin_routed）由 Quoin 编排、Stele 网关执行，supervisor 只转发结果。Plinth **MUST NOT** 保存跨 Attempt Session、checkpoint、Agent memory 或工作区，也 **MUST NOT** import 插件体系与 Quoin 包（编译级零感知）。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[ADR-0011](../../../adr/0011-component-responsibility-and-plugin-v2.md)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-WORKER-002 —（ADR-0011 修订）** supervisor 独占模型 provider API key（FetchCredentialGrant 仅 model_provider）；worker 只能接收冻结的非秘密输入、一次性工作区、固定工具 schema 和本地协议。秘密 **MUST NOT** 进入 worker 环境、参数、工作区、模型上下文、Artifact、Evidence 或普通日志。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
- **ARCH-WORKER-003 —** worker 只拥有 `bash`、`read`、`write`、`grep` 等无凭据工作区工具。`bash` Tool **MUST** 在 worker 隔离边界内以固定 cwd 执行 `/bin/bash --noprofile --norc -c <command>`，不继承 supervisor 环境；可执行文件、版本、绝对路径与 Landlock 只读运行时路径只由 [`contracts/plinth-worker-tools.yaml`](contracts/plinth-worker-tools.yaml) 定义，运行时不得安装/下载工具。（ADR-0011 修订）模型调用由 supervisor 直连 provider；插件与 Artifact 类型化工具（quoin_routed）由 Quoin 执行并经 `ExternalToolResult` 帧推回，supervisor/worker 都不执行外部平台调用；worker **MUST NOT** 获得 kubeconfig、外部通用 HTTP、Quoin token 或任意连接选择接口。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[Issue #17](https://github.com/Suknna/quoin/issues/17) Q17.35 D、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-WORKER-007 —** v1 在 Kubernetes Restricted PSS 下让 supervisor/worker 同容器、同 uid 运行，并由 worker 在处理任何 Attempt 输入前 fail-closed 建立 `no_new_privs`、Landlock ABI >= 6（只允许既定只读运行时路径与当前工作区，并限制 signal 和 abstract Unix socket）及进程内 seccomp（拒绝外部网络与 `ptrace`/`process_vm_*`/`kcmp`/`pidfd_getfd` 等跨进程能力）；Landlock domain hierarchy 还会隐式限制 sandboxed worker 对域外 supervisor 的 ptrace 关系，但不是可配置 ptrace allowlist；`PR_SET_DUMPABLE=0`、相同 uid 下的 Yama/procfs 边界、显式拒绝跨进程 syscall 的 seccomp 与真实负向自检共同形成防御纵深。supervisor **MUST** 清空 worker 环境、设置自身不可 dump、只继承 stdin/stdout/stderr、使用独立进程组和仅本 Attempt 可写工作区；内核必须 >= 6.12 且启用 Landlock。v1 **MUST NOT** 引入 user namespace、bubblewrap、额外 worker daemon 或第二套本地协议。（来源：Issue #13 Q13.4 A、Linux Landlock/Yama/procfs、Go `os/exec` 与 Kubernetes Restricted PSS 研究）
...
| --- | --- | --- | --- | --- |
| Initial Analysis | 是 | 最小顺序 loop | 已启用接入的插件工具（`thanos_query` 只读）与 Artifact 工具 | 冻结 Occurrence、已启用接入输入项、已有 Evidence；输出 Initial Analysis |
| Investigation | 是 | 最小顺序 loop | 已启用接入的插件工具、Artifact 工具与工作区工具 | 当前有效消息分支、来源引用、Artifact；输出 assistant Message |
| Inspection Analysis | 是 | 固定分析调用，可使用工作区工具 | **不重新查询 Thanos** | 本 Run 冻结 Evidence；输出不可变 Report |
...
| Connection Probe | 否 | 无 Agent/Tool loop | （ADR-0011）Quoin 本地编排，经 Stele 网关按冻结 action set 执行真实黑盒探测 | 输出不可变 typed probe result；不产生领域分析 |
...
- **ARCH-MODE-004 —** 多 Tool Call **MUST** 按模型响应顺序逐个持久化和执行；同一 Attempt 内不得并行执行模型返回的 Tool Call。来源级调用（ADR-0004）按 Attempt 快照冻结的已启用接入解析：显式 `sourceRef` 命名唯一来源，歧义返回预检结果，绝不首项获胜或扇出全部来源；历史业务绑定 Attempt 继续按其冻结绑定解释。模型不得选择或提交连接。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-CHAT-003 —** 模型从 Attempt 快照冻结的已启用接入输入项和用户自然语言识别人类领域目标，只能在 Tool 参数中提交 `sourceRef`、对象 locator 或历史业务系统 key/名称等人类可读定位；模型 **MUST NOT** 查看或选择 Connection、ConnectionRevision、CredentialGeneration、grant 或凭据。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[ADR-0004](../../adr/0004-plugin-capability-registry.md)）
- **ARCH-CHAT-004 —** Quoin 是路由权威：来源级主线（ADR-0004）下，Tool Call 事务从 Attempt 快照冻结的已启用接入解析来源，并在同一事务冻结受权接入 revision/credential generation。显式 `sourceRef` 必须唯一命中；缺失、停用或歧义必须返回结构化预检结果，**MUST NOT** 猜测、回退到全局或选择第一个接入。历史业务声明的显式授权引用只用于解释旧 Attempt，不派生新授权。（来源：[ADR-0004](../../adr/0004-plugin-capability-registry.md)）
- **ARCH-CHAT-005 —** 指标查询 Tool（`thanos_query`）只接受 `query` 与可选 `sourceRef`；Quoin/supervisor 按冻结 grant 注入接入端点、认证与范围约束并校验参数，模型不得提供 raw endpoint、认证头或接入/连接 ID 来绕过边界。历史声明 Attempt 的强制 matchLabels 校验仅按其冻结声明解释。（来源：[ADR-0004](../../adr/0004-plugin-capability-registry.md)）
...
- **ARCH-TOOL-001 —** 模型返回的全部 Tool Call 必须先按响应顺序在一个 Quoin 事务中创建为 `pending`，然后 worker 才收到可继续响应；任何本地或 supervisor 执行都不得早于持久 Tool Call ID。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
- **ARCH-TOOL-002 —** worker 必须按 PreparedToolCall 顺序发送本地协议 `ExecuteToolCall`；supervisor 经 `BeginToolCall → BeginToolCallAck` 将既有 pending Tool Call 推进 running 后回 `ToolCallStarted`。本地工具收到该许可后才执行，大输出由 supervisor 上传 Artifact；类型化外部工具由 supervisor 执行。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-TOOL-005 —** Knowledge 查询只返回已确认且当前可复用 KnowledgeVersion locator 与正文投影；它不产生 Evidence。Thanos 观测产生 Evidence；本地工作区计算仅在其输出需要跨 Attempt 存活时产生 Artifact。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-RECOVERY-002 —** fence 后不得开始新 Model/Tool Call。Attempt、active Model Call 与 pending/running Tool Call 必须由一个 Quoin 终态事务闭合；任何迟到子结果都不得把父 Tool Call 或父 Attempt 改回成功。迟到 provider/tool 结果按提交顺序只形成允许的审计/Artifact/Evidence，不得创建成功消息或 Report。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
- **ARCH-RECOVERY-006 —** `CompleteModelCallAck` 丢失时 supervisor 以相同 model_call_id/digest/outcome 重放，Quoin 从已封存 Model Call 与 Tool Call IDs 重建同一 Ack；内容变化必须冲突。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)）
...
- **ARCH-OBS-002 —** 指标语义至少覆盖 active worker、按权威 Attempt type/terminal reason 的 Attempt、Model/Tool latency 与 retry、token usage、Artifact spill bytes、context-evicted turns、sandbox launch failures、control reconnect 与 cancellation latency；精确 family/type/label/bucket 只能由 [`contracts/metrics.yaml`](contracts/metrics.yaml) 定义，代码不得另建动态 label。指标不得包含用户正文、Prompt、Tool 参数、Artifact path 或凭据。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[Issue #17](https://github.com/Suknna/quoin/issues/17)、OPS-METRIC-*）
...
- **ARCH-OBS-004 —** Quoin、Plinth、Stele 三个后端组件的用户/Runtime 数据面与内部运维面必须使用不同固定监听器；`/livez`、`/readyz`、`/metrics` 只存在于运维监听器，网络可达性与探针时序由 [operations.md](operations.md) 的 OPS-NET-* / OPS-HEALTH-* / OPS-K8S-* 独占定义。（来源：[Issue #17](https://github.com/Suknna/quoin/issues/17) Q17.6/Q17.17）
...
- **ARCH-VALIDATION-002 —** Plinth 实现必须对抗验证：Tool 持久化前不得执行；取消先/结果先两种提交序；部分 token 后失败；HTTP 观察者断开；ResultAck 丢失重放；连接轮换并发；Artifact 上传/Read/Grep 中断；正文 GC；worker 越权读取 `/proc`/状态目录/网络；Succeeded Attempt 拒绝任何 active Model/Tool Call；Tool protocol group 上下文切割。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
