...
- **ARCH-BROWSER-005 —** Quoin 是全局浏览器容量队列与身份锁的唯一裁决者；Lintel 只声明固定 slot 总数并报告占用。所有四类容量请求在派发前先创建同一 `browser_operations` 行，容量不足只按 `browser_operations.id` 的 SQLite 提交创建序使用全局 FIFO，`requestedAt` 只作显示时间；Inspection Run/Config Verification Run 的等待提示直接由该行投影，不参与调度。不设租户配额、预留或抢占；同一 Browser Identity 已占用则立即返回 `IdentityBusy`，不得进入另一条无界身份队列。（来源：[Issue #14](https://github.com/Suknna/quoin/issues/14)、[CONTEXT.md](../../../CONTEXT.md)）
