...
- **ARCH-COMP-004 —** Lintel 只执行 Quoin 明确创建的 Browser Operation，浏览器 profile 与 Browser Identity 由既有 Lintel 契约拥有；Plinth 请求浏览器工具时只能经 Quoin 转发。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
...
## 9. Tool Call、Evidence 与 Artifact 提交顺序
...
## 10. 输入快照、连接 grant 与 Artifact 读取
...
- **ARCH-RECOVERY-004 —** ResultProposal 与 ResultAck 丢失时，Plinth 使用相同 Attempt/digest 重放；Quoin 只在同一事务提交最终领域输出、全部引用与 Attempt `Succeeded`，相同 digest/outcome 的重复 proposal 从已持久结果重建并返回与首次完全相同的 Ack；不同 digest/outcome 的重放是协议冲突。（来源：[Issue #13](https://github.com/Suknna/quoin/issues/13)、[CONTEXT.md](../../../CONTEXT.md)）
