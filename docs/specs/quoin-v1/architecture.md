...
| Connection Probe | 否 | 无 Agent/Tool loop | supervisor 按连接类型的冻结 action set 执行真实黑盒探测 | 输出不可变 typed probe result；不产生领域分析 |
