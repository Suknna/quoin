...
`quoin-data` 保存 SQLite 与 Artifact，`quoin-backups` 保存核心一致性备份，`plinth-state` 与 `lintel-state`（仅在显式启用浏览器时创建）保存各 Runtime 的长期状态。不要删除或替换这些卷而不执行已批准的恢复流程。丢失 `quoin-data` 意味着权威业务数据丢失；丢失根密钥时，必须停机、独占数据库并执行 `quoin maintenance rebind-root-key`，随后重新录入受影响连接。丢失 Runtime 状态卷时，由 Admin 为原 Runtime slot 准备替换注册；不得创建额外 slot。普通 SQLite 备份不包含部署外部的 Stele service token、TLS 私钥或根密钥，Secret 必须由运维独立备份并与数据匹配保管。
...
组件集不含 Lintel：浏览器业务已退役，浏览器镜像的 Chromium 依赖不再构建（历史构建方式见 `deploy/retired/`）。`QUOIN_IMAGE_NAMESPACE`、`QUOIN_IMAGE_TAG`、`QUOIN_IMAGE_VERSIONS` 与 `QUOIN_IMAGE_GOPROXY` 可按发布流程覆盖。
