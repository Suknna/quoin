...
1. **在你的插件包内声明实现**（不要改 attempt 核心；示例为节选，完整可编译版见 `internal/plugins/example_plugin_test.go`）：
   ...
   ```
2. **插件包自持声明与实现**：你的插件包同时拥有 Descriptor 与 ToolDef；工具的 registry 声明**从你的 ToolDef 派生**，勿手抄第二份：
   ```go
   declaration, err := def.DescriptorTool() // name/version/location/failureMode/描述/参数schema 全部派生
   ```
   将其放入你的 `plugins.Descriptor.Tools`，经共享 builtin 声明源进入各进程的同一 `plugins.Registry`；描述符能力仅宣称有真实绑定的能力（`tools` ⇔ 目录非空；`execute_tool`/`probe`/`discover`/`collect` 必须先有绑定）。`attempt.BuildCatalogs(registry, implementations, enabled)` 装配目录与实现索引并双向校验（声明必有实现、实现必有声明），失败即启动失败。
...
- [ ] 工具名全局唯一；描述符工具字段用 `ToolDef.DescriptorTool()` 从实现派生，无手抄副本
- [ ] 声明与实现随插件包进入共享 builtin 声明源，无第二工具表、无 init 旁路注册；执行绑定能力集合与非 nil 接口精确相等（描述性能力不进 bundle）
