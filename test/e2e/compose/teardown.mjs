...
export default async function globalTeardown() {
  ...
  const compose = join(stack, 'state', 'quoin', 'compose', 'generated', 'compose.yaml'), resources = [], failures = []
  const shell = (command) => { try { execSync(command, { cwd: root, stdio: 'pipe', shell: '/bin/bash' }); return 0 } catch (e) { return e.status ?? 1 } }
  // A failed Docker client command is not evidence that its target is absent.
  // Keep cleaning independently, but preserve that uncertainty in the verdict.
  const dockerReady = shell('docker info >/dev/null 2>&1') === 0
  if (!dockerReady) failures.push('Docker daemon is unavailable; Docker resource absence cannot be verified')
  const removeProbe = (name, kind, removalCommand, probeCommand, dockerResource = true) => {
    const removeExitCode = shell(removalCommand), probeExitCode = shell(probeCommand)
    const absent = dockerResource ? dockerReady && probeExitCode !== 0 : probeExitCode !== 0
    resources.push({ name, kind, removalCommand, removeExitCode, probeCommand, probeExitCode, observedFinalState: absent ? 'absent' : dockerResource && !dockerReady ? 'unknown' : 'present' })
    if (removeExitCode !== 0 || !absent) failures.push(`cleanup failed for ${name}: remove=${removeExitCode} probe=${probeExitCode}`)
  }
  if (existsSync(compose)) {
    if (browserTicket) removeProbe('ticket authentication fixture', 'container', `docker rm -f ${fixture} >/dev/null 2>&1 || true`, `docker container inspect ${fixture} >/dev/null 2>&1`)
    if (!browserTicket) {
  ...
  const awaitGone = async (name, file) => {
    ...
    if (!absent) failures.push(`cleanup failed for ${name}: still running after SIGKILL`)
  }
  ...
  try {
    ...
    failures.push(`remove stack directory: ${error instanceof Error ? error.message : String(error)}`)
  }
  const stackProbe = shell(`test -e ${stack}`)
  resources.push({ name:'stack directory, TLS keys/cert, generated config, admin password, session cookie, browser profile state', kind:'file-tree', removalCommand:`rm -rf ${stack}`, probeCommand:`test -e ${stack}`, probeExitCode:stackProbe, observedFinalState: stackProbe !== 0 ? 'absent' : 'present' })
  if (stackProbe === 0) failures.push('stack directory remains')
  const phase = process.env.QUOIN_TEARDOWN_PHASE ?? 'playwright-global-teardown'
  ...
  merged.phases.push({ phase, resources, failures })
