#!/usr/bin/env bash
# Verbatim acceptance run for ticket #26 (T03). Preserves pipeline exit codes
# and writes all evidence under .artifacts/tickets/T03.
set -euo pipefail
repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)
cd "$repo_root"
EVIDENCE=.artifacts/tickets/T03
rm -rf "$EVIDENCE"
mkdir -p "$EVIDENCE"
...
run ticket-acceptance env QUOIN_EVIDENCE_DIR="$PWD/$EVIDENCE" \
  go test -json ./... -run '^TestTicket03' -count=1
...
run playwright env QUOIN_EVIDENCE_DIR="$PWD/$EVIDENCE" \
  pnpm --dir web exec playwright test --grep '@ticket-03' --project=chromium
