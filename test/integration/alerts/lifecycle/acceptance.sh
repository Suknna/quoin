#!/usr/bin/env bash
# Verbatim acceptance run for ticket #27 (T04). Preserves pipeline exit codes
# and writes all evidence under .artifacts/tickets/T04.
set -euo pipefail
repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)
cd "$repo_root"
EVIDENCE=.artifacts/tickets/T04
rm -rf "$EVIDENCE"
mkdir -p "$EVIDENCE"
...
run ticket-acceptance env QUOIN_EVIDENCE_DIR="$PWD/$EVIDENCE" \
  go test -json ./... -run '^TestTicket04' -count=1
...
run playwright env QUOIN_EVIDENCE_DIR="$PWD/$EVIDENCE" QUOIN_TICKET=T04 \
  pnpm --dir web exec playwright test --grep '@ticket-04' --project=chromium
