// Package kubernetes hosts the T07 kubernetes slice: the real compose stack
// with a registered Plinth supervisor probing the host's real k3s cluster
// through the full read-capability action set, plus deterministic negative
// fixtures (unparseable kubeconfig, wrong server). Evidence lands under
// .artifacts/tickets/T07/.
package kubernetes
...
func prepareAndReveal(t *testing.T, client *http.Client, base, origin, slot string, expectedRow float64) registrationToken {
	t.Helper()
	prepare := doRequest(t, client, http.MethodPost, base+"/api/v1/runtime-slots/"+slot+"/registration/prepare", origin, fmt.Sprintf(`{"clientCommandId":"t07k-%s-%d","expectedRowVersion":%d}`, slot, time.Now().UnixNano(), int64(expectedRow)))
...
// stackGateway resolves the compose network's gateway address (the host on
// that bridge) so containers reach the host k3s API server.
...
// hostKubeconfig rewrites the host k3s kubeconfig so containers reach the
// API server through the stack network's gateway instead of 127.0.0.1.
func hostKubeconfig(t *testing.T) string {
	t.Helper()
	raw, err := os.ReadFile("/etc/rancher/k3s/k3s.yaml")
	if err != nil {
		t.Fatalf("host k3s kubeconfig unavailable: %v", err)
	}
	rewritten := strings.ReplaceAll(string(raw), "https://127.0.0.1:6443", "https://"+stackGateway(t)+":6443")
	// The gateway address is not in the k3s certificate SAN list; the
	// fixture trusts it explicitly (cluster identity itself is unchanged).
	return strings.Replace(rewritten, "certificate-authority-data:", "insecure-skip-tls-verify: true\n    certificate-authority-data:", 1)
}
